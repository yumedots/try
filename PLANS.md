...
The extracted tree is stamped with the md5 of the tarball it came from (`.cache/root.stamp`), so it is reused while that tarball is current and rebuilt when upstream rebuilds `latest` - without the stamp a new tarball was silently built from the old tree. When the tree is stale the download does not finish before the extraction starts: the ranges are still fetched in parallel, each writes a marker when it lands, and a reader consumes them in order into `tee` (the tarball) and on into `pigz -dc | tar -x`, so the 8 s extraction runs under the ~85 s download and the tarball is still written intact. It extracts into `.cache/root.new`, and that tree is only moved into place after the md5 of the finished file matches, so no disk is ever built from bytes that failed the checksum; a half-extracted staging tree is dropped and the plain extraction from the kept tarball runs instead. Anything that needs the tree itself (`xmake guest`) calls the same `extract_tree`, which reuses the stamped tree or extracts from the tarball.
...
Both halves of that have to exist before the greeter's X server does, and the service cannot assume they do: on a cold boot the EDID it finds first is the boot mode (no host window yet), so there is no scale to write yet, and the greeter's X server starts with no `/run/try/` at all - `Xsetup` then finds no config, exits early, and the login screen renders at 1x with nothing listening for the update that arrives seconds later. So the service now writes the greeter's resource and daemon config at startup whatever it has (1x if it has nothing yet, corrected on the first real window) and, when it pushes a new value, starts `xsettingsd` itself if the display has none - measured on a cold boot: `Xft.dpi: 192 via Xsetup` from the daemon the hook found already running, and `/run/try/xft.dpi` following a resize live (192 -> 128 -> 230) without restarting the greeter.
...
`yumedots/qemu` holds that tree (`261e32a`, one commit) plus our commits, so the artifact is pinned by commit sha. The pin is `1100bc8384`, which is both "send the window's size at launch" and "let the scale shrink with the window" - a bottle from before those two boots the guest at the boot mode (the host screen) until the window is first resized, which `zoom-to-fit` then draws scaled down, so the login screen comes up small. That is what a stale pin looks like from the guest, and it is worth a re-pin whenever the fork moves:
...
- **QEMU:** its D-Bus listener delivers a GL scanout only under `CONFIG_GBM` (Linux dmabuf) or `WIN32` (D3D11 shared texture). On macOS that branch is empty. The texture borrowing in the patched tree is in-process only — it feeds the cocoa frontend, not a client across a socket. A macOS delivery needs a handle that survives a process boundary. Not the `IOSurface` id: `IOSurfaceLookup` fails across processes on macOS 26, with or without `kIOSurfaceIsGlobal` (measured). The surface's mach port is the handle that travels: the client `bootstrap_register`s `IOSurfaceCreateMachPort(surface)` under a name, QEMU `bootstrap_look_up`s it and takes the surface back with `IOSurfaceLookupFromMachPort` (measured working).
