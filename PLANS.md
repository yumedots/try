...
The guest has no display manager: `guest/hyprland.service` autologs the guest user into Hyprland on tty1 (`User=alarm` with `PAMName=login`, which is what makes logind hand it a real graphical session - `XDG_RUNTIME_DIR` and a session type). `Restart=always` means a compositor that dies is back a second later instead of leaving the screen on the last frame it drew. There was a greeter (SDDM) before this, and it was the only reason for the scale plumbing it needed: Qt 6 on X11 ignores RandR physical DPI and takes a 96 dpi screen as 1x, so the whole device pixel ratio was the DPI the display had to be told, which is what `guest/Xsetup`, `/run/try/xft.dpi`, `/run/try/xsettingsd.conf` and `tryDisplay`'s greeter half existed to do. A Wayland session reads the scale from the compositor, so all of it is gone with the display manager.
